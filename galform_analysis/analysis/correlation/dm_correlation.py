...
def _normalize_positions_units(
    ...
    if boxsize > 1.0e4:
        warnings.warn(
            f"Large boxsize detected ({boxsize:.1f}); assuming kpc/h and "
            "converting to Mpc/h."
        )
        return positions / 1000.0, boxsize / 1000.0
        ...
        with h5py.File(tree_file, "r") as f:
            ...
            if mhalo_min is not None:
                ...
                if mass is not None:
                    ...
                    warnings.warn(
                        "Could not find mass field for mass cut. "
                        "Proceeding without cut."
                    )
            boxsize = None
            ...
            if boxsize is None:
                ...
                boxsize = float(np.max(extent))
                warnings.warn(
                    f"BoxSize not found in file, estimated from extent: {boxsize:.1f}"
                )
...
def _load_halo_positions_from_binary(
    ...
    """Load dark matter halo positions from binary merger tree files.

    (Millennium format). This is a placeholder for binary tree formats.
    Implementation depends on the specific binary format used (e.g., old
    Millennium trees).
...
def dm_correlation_from_tree_file(
    ...
    Args:
        ...
        boxsize_override: If set, use this boxsize (Mpc/h) instead of the tree
            file boxsize

...
def compute_3pcf_counts_reference(positions, rbins, boxsize=542.16):
    N = len(positions)
    nbins = len(rbins) - 1
    ...
    return {"r": 0.5 * (rbins[:-1] + rbins[1:]), "t_total": t_total}
...
if __name__ == "__main__":
    ...
    # chunks or just sample.
    # Better: just load a few hundred to get 5000 galaxies.
    df = load_subvolume_galaxies(
        base_dir, iz, ivols[:100], k_total=k, mstar_min_log10=mstar_cut
    )

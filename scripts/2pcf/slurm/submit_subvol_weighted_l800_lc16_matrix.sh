#!/bin/bash
# Submit subvolume-weighted-estimator 2PCF jobs for L800/lc16 over requested
# redshifts, halo-mass cuts, centrals-only settings, and n_subvol values.
...
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/data/halo_sampling_4}"
MODE="${MODE:-weighted}"
...
# Requested redshifts and matrix definition (same as prior halo-sampling matrix run).
IZ_LIST=(271 207 155)
...
submit_array_job() {
    ...
    sbatch \
        ...
        --job-name="${job_name}" \
        --array="${array_spec}" \
        --export=ALL,MODE="${MODE}",PARTITION_SCHEME="${PARTITION_SCHEME}",SIM_NAME="${SIM_NAME}",MODEL_NAME="${MODEL_NAME}",IZ="${iz}",NMAX="${nmax}",K_TOTAL="${K_TOTAL}",OUTPUT_DIR_BASE="${out_dir_base}",BOXSIZE="${BOXSIZE}",MHALO_MIN="${mhalo}",CENTRALS_ONLY="${centrals}",BASE_DIR_OVERRIDE="${BASE_DIR}" \
        "${TEMPLATE}"
...
submit_single_job() {
    ...
    sbatch \
        --partition="${PARTITION}" \
        --job-name="${job_name}" \
        --export=ALL,MODE="${MODE}",PARTITION_SCHEME="${PARTITION_SCHEME}",SIM_NAME="${SIM_NAME}",MODEL_NAME="${MODEL_NAME}",IZ="${iz}",NMAX="${nmax}",K_TOTAL="${K_TOTAL}",SUBVOLS="${n}",OUTPUT_DIR="${out_dir}",BOXSIZE="${BOXSIZE}",MHALO_MIN="${mhalo}",CENTRALS_ONLY="${centrals}",BASE_DIR_OVERRIDE="${BASE_DIR}" \
        "${TEMPLATE}"
...
submit_spec() {
    ...
    local base_job="swL8lc16_i${iz}_N${nmax}_${set_tag}_${mhalo_tag}_c${centrals}"

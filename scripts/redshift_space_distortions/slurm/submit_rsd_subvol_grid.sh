...
SUBVOLS_CSV="${SUBVOLS_CSV:-10,20,50,100,400,800}"
...
for mode in "${MODES[@]}"; do
  for n in "${SUBVOLS[@]}"; do
    JOB_NAME="rsd_${MODEL_NAME}_${SIM_NAME}_iz${IZ}_${mode}_n${n}"
    ...
    sbatch \
      --job-name="${JOB_NAME}" \
      --partition=cosma5 \
      --export=ALL,SIM_NAME="${SIM_NAME}",MODEL_NAME="${MODEL_NAME}",IZ="${IZ}",NMAX="${NMAX}",N_SUBVOL="${n}",MODE="${mode}",BOXSIZE="${BOXSIZE}",MHALO_MIN="${MHALO_MIN}",N_RANDOM="${N_RANDOM}",CENTRALS_ONLY="${CENTRALS_ONLY}",BASE_DIR="${BASE_DIR}",OUTPUT_DIR="${OUTPUT_DIR}" \
      "${TEMPLATE}"
    total=$((total + 1))
  done
done

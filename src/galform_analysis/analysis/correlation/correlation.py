...
def notebook_style_correlation_for_nvolumes(
    base_dir: str,
    iz_num: int,
    ...
    """Compute notebook-style standard xi(r) for each requested N_subvol.

    This mirrors ``kai/kai_corrfunc.ipynb`` preprocessing and uses halotools
    ``tpcf`` after loading galaxies from ivols ``[0, ..., N-1]``.
    """
    if rbins is None:
    ...
    for nvol in nvolumes_list:
        ...
        xi = compute_notebook_style_standard_xi(
            gals,
            rbins=rbins,
            boxsize=boxsize,

...
def compute_smf_avg_by_snapshot(iz_path: str, 
                                bins: np.ndarray = None,
                                ivol_sample: Optional[int] = None,
                                random_seed: int = 42) -> Optional[Dict[str, Any]]:
    """Compute stellar mass function by averaging across subvolumes.
    
    This function iterates through all subvolumes in a snapshot, computes
    the stellar mass function for each, and returns the average with
    standard deviation.
    ...
    # Sample if requested
    if ivol_sample is not None and ivol_sample > 0:

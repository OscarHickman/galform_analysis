...
def compute_3pcf_counts_with_scope(
    ...
    rbins: (nbins,) distance bins for the sides of the triangle (simplified to equilateral/isosceles for this test)
    """
    N = len(positions)
    nbins = len(rbins) - 1
    ...
    # To keep it simple for this test: we count triplets (i,j,k) where dist(i,j) and dist(i,k) are in the same bin
    for i in range(N):
        if i % 1000 == 0:
        ...
        for j_idx, j in enumerate(idx):
            ...
            # Check periodic wrap manually if not handled by norm? cKDTree handles wrap in query.
            ...
            for k in idx[j_idx + 1:]:
                ...
                # For this test, we only look at "v-configurations" where both arms are in the same bin
                if bin_j == bin_k:
...
if __name__ == "__main__":
    import sys
    from src.readers.loaders import read_snapshot_data
    ...
    ivols = list(range(m))
    df = load_subvolume_galaxies(base_dir, iz, ivols, k_total=k, mstar_min_log10=mstar_cut)
    ...
    results = compute_3pcf_counts_with_scope(pos, labels, rbins, m_selected=m, k_total=k)
